...
private:
	...
	//TODO: use libshaderc to directly compile shaders from code
	...
	void createSwapChain() {
		...
		// TODO: Ensure that adding only a single more image in swap chain gives desired performance
		...
		// TODO: May need to use VK_IMAGE_USAGE_TRANSFER_DST_BIT if doing post processing first and use memory operation to transfer the img to a swap chain
		createInfo.imageUsage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;

		QueueFamilyIndices indices = findQueueFamilies(physicalDevice);
		uint32_t queueFamilyIndices[] = { indices.graphicsFamily.value(), indices.presentFamily.value() };

		// TODO: Switch over to SHARING_MODE_EXCLUSIVE for differing queue families
		...
		createInfo.clipped = VK_TRUE; // May impact predicatbility if reading pixels back obscured by other windows

		// TODO: Add support for recreating swap chain if window moves, etc.
	...
	VkSurfaceFormatKHR chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
		...
		// TODO: If the desired color format is not available add ranking in for how good the options are
	...
	void cleanup() {
		...
		if (enableValidationLayers) {
			DestroyDebugUtilsMessenerEXT(instance, debugMessenger, nullptr);
		}
